...
- **Two pieces integrated and committed so far:**
  ...
  2. First-visit intro preloader (hand-built "double stairs" reveal, inspired by Skiper UI's paid `skiper10` component which this project has no license for) — text reveal → fade to black → 8-column staggered stairs uncover the site, once per tab via `sessionStorage`. Commit `9b60780`. Full detail including the "intro finish looked like a page reload" bug (root cause + fix: two independent React roots instead of one shared tree) — see [[2026-08-13]] (session 2).
- **Immediate next step, session start:** ask the user whether they've done their own manual look at the intro preloader in a real (non-automated) browser tab yet — last session's own verification was thorough (console-log instrumentation, computed-style checks, slowed-down visual captures) but was run through Claude-in-Chrome automation, which has a known quirk (documented below) that doesn't affect real users but is worth being upfront about. If they have feedback, that's the starting point; if not, offer to start the dev server so they can look.
- **After that:** resume the plan's per-component workflow (`knowledge/frontend-plans/ui-redesign-reactbits.md`, "Per-component workflow" section) for whatever component the user shares next — from React Bits (`@react-bits` registry, the established path) or another source (as with `skiper10`, evaluate case-by-case; paid/gated components need explicit user sign-off before either buying a license or hand-building a lookalike, don't assume).
- **Patterns established across both sessions, expected to recur — don't re-litigate each time (but still flag+ask if a genuinely new situation comes up):**
  ...
  - If a new component needs to coexist with something else that must never be interrupted (e.g. another persistent background, a global timer), consider mounting it on its own independent `createRoot()` rather than as a sibling in a shared tree — sibling position in a fragment is not a reliable guarantee against remounts in practice (confirmed empirically this session, exact React internals not fully chased — verify with mount/unmount logging if it matters, don't assume).
...
- `node_modules/` at the repo root and `.agents/` remain untracked in git (pre-existing, not created by this workstream) — left alone across both sessions; revisit only if the user asks.

## Links

- Latest daily log: [[2026-08-13]] (session 1: React Bits islands bootstrap + FloatingLines background; session 2: first-visit intro preloader with double-stairs reveal, and the two-React-roots fix for a background-remount bug)
...
- Active frontend workstream: [[ui-redesign-reactbits]] (`knowledge/frontend-plans/ui-redesign-reactbits.md`) — approved 2026-08-11, bootstrap + FloatingLines done 2026-08-13 (session 1), intro preloader done 2026-08-13 (session 2), in progress on `feature/ui-redesign-reactbits`.
